...
void gps_time_nmea_read(gps_time_t *pointer, const char *sentence) {
    ...
    switch(token_count) {
      case 0: {
        if(strlen(nmea_token)==5) {
          ...
          } else if(strcmp(nmea_token + 2, "VTC") == 0) {
      ...
      case 1: {
        if(sentence_type == GPS_TIME_SENTENCE_TYPE_GPGGA || sentence_type == GPS_TIME_SENTENCE_TYPE_GPRMC) {
          sscanf(nmea_token, "%02" PRIu32 "%02" PRIu32 "%02" PRIu32, &(pointer->hour), &(pointer->minute), &(pointer->second));
      ...
      case 8: {
        if(sentence_type == GPS_TIME_SENTENCE_TYPE_GPRMC) {
          sscanf(nmea_token, "%02" PRIu32 "%02" PRIu32 "%02" PRIu32, &(pointer->day), &(pointer->month), &(pointer->year));
  ...
  if(((pointer->status & GPS_TIME_READY) == GPS_TIME_READY)) {
    ...
    if(pointer->lag == 0) {
      ...
      printf("Time of day will be set: %s", ctime(&secs));
...
void gps_time_free(gps_time_t *pointer) {
  free(pointer);
}
